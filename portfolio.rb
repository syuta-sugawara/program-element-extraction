...
# 第1引数をポートフォリオファイルパスとして，ファイル読み込み，
# 改行コード(\n)をデリミタとして，1行ずつに分割した配列aDataに格納
aData = File.read(ARGV[0]).split("\n")
hData = {}      # ポートフォリオ解析データ
...
# ・引数が5つ以上ある場合，何もしない
case ARGV.size()
# 引数の数が2つの場合，大見出しのみ指定されたと見なす
when 2
  # 第2引数に指定された文字列がhDataのキー(大見出し)として存在し，:TEXTにデータがある場合出力する
  if (hData.has_key?(ARGV[1]) && (hData[ARGV[1]][:TEXT] != ""))
    puts hData[ARGV[1]][:TEXT]
  end
# 引数の数が3つの場合，大見出しと中見出しが指定されたと見なす
when 3
  # 第2引数に指定された文字列がhDataのキー(大見出し)として存在する場合のみ処理する
  if (hData.has_key?(ARGV[1]))
    # 第3引数に指定された文字列がhData[ARGV[1]]のキー(中見出し)として存在し，:TEXTにデータがある場合出力する
    if (hData[ARGV[1]].has_key?(ARGV[2]) && (hData[ARGV[1]][ARGV[2]][:TEXT] != ""))
      puts hData[ARGV[1]][ARGV[2]][:TEXT]
    end
  end
# 引数の数が4つの場合，大見出し・中見出し・小見出しが指定されたと見なす
when 4
  # 第2引数に指定された文字列がhDataのキー(大見出し)として存在する場合のみ処理する
  if (hData.has_key?(ARGV[1]))
    # 第3引数に指定された文字列がhData[ARGV[1]]のキー(中見出し)として存在する場合のみ処理する
    if (hData[ARGV[1]].has_key?(ARGV[2]))
      # 第4引数に指定された文字列がhData[ARGV[1]][ARGV[2]]のキー(小見出し)として存在し，:TEXTにデータがある場合出力する
      if (hData[ARGV[1]][ARGV[2]].has_key?(ARGV[3]) && (hData[ARGV[1]][ARGV[2]][ARGV[3]][:TEXT] != ""))
        puts hData[ARGV[1]][ARGV[2]][ARGV[3]][:TEXT]
      end
    end
  end
end
